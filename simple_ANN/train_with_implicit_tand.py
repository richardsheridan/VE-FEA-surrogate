...
# define the train process that include calculating tan delta for MSE loss along
# with E' and E"
def train(model, x, y, optimizer, criterion, output_split_dim):
    # clear the gradients of all optimized variables
    model.zero_grad()
    # forward pass: compute predicted outputs by passing inputs to the model
    output = model(x)
...
def run_training(
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in tqdm(train_loader):
            ...
            loss, predictions = train(model,x_train,y_train,optimizer,criterion,output_split_dim)
        ...
        for batch in tqdm(valid_loader):
            ...
            ep_epp_tand = torch.cat((output,output[...,output_split_dim:]/output[...,:output_split_dim]),-1)
            # calculate the loss
            loss = criterion(ep_epp_tand,y_val)
    ...
    for batch in tqdm(test_loader):
        ...
        ep_epp_tand = torch.cat((output,output[...,output_split_dim:]/output[...,:output_split_dim]),-1)
        # calculate the loss
        loss = criterion(ep_epp_tand,y_test)
        # update running validation loss 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--loss_fn", type=str, default='mse',
        ...
        help='save model and history file under this name')
    parser.add_argument("--task_name", type=str, default='implicit_tand',
